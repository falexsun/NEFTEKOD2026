...
- Внешние оповещения — демонстрация в Telegram, с возможностью подключения
  другого мессенджера или корпоративного канала через адаптер/webhook
...
5. **Внешнее уведомление:** при прогнозе превышения оператор может получить
   алерт с текущим Q21, прогнозом, вероятностью, интервалом неопределённости и
   временем ожидаемого события. На снимке Telegram используется только как
   наглядный пример. Канал доставки не является частью модели: для промышленной
   эксплуатации вместо Telegram можно подключить любой согласованный мессенджер,
   электронную почту или систему управления инцидентами через соответствующий
   адаптер/webhook. Конкретный коннектор и правила эскалации настраиваются при
   внедрении.

   <p align="center">
     <img src="docs/screenshots/telegram-alert-mobile.png" width="420" alt="Пример тестового уведомления о прогнозе превышения Q21 в Telegram">
   </p>

   На изображении показано тестовое сообщение с синтетическими значениями;
   адрес `localhost` предназначен только для демонстрационного стенда.
