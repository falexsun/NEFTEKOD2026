...
export function OperatingModes({ policy }: { policy?: OperatingPolicy }) {
  ...
  return <section className="panel operating-modes" aria-label="Режимы установки">
    <div><span className="eyebrow">ДОПУСК К РАСЧЁТУ</span><h2>{policy.allowed ? 'Рабочие режимы подтверждены' : policy.disposition === 'NO_ACTION' ? 'Оптимизация приостановлена' : 'Недостаточно данных о режиме'}</h2><p>{policy.allowed ? 'Дополнительно требуются готовность модели и полная история.' : policy.reasons.join(' · ')}</p></div>
