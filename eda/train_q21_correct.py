"""
Correct Q21 training - final fix for persistence baseline calculation.
"""
import argparse
import json
...
def prepare_features(data: pd.DataFrame, horizon_hours: int) -> pd.DataFrame:
    ...
    # Adaptive windows based on horizon
    if horizon_hours <= 3:
        short_w, medium_w, long_w = [1, 2, 3], [6, 12, 24], [48, 72]
    else:
        short_w, medium_w, long_w = [2, 4, 6], [12, 24, 48], [72, 96]
    ...
    # Persistence forecast for THIS horizon
    features["persistence"] = target  # Current value is persistence forecast
...
def train_horizon(data: pd.DataFrame, horizon: int, output_dir: Path) -> Dict:
    ...
    valid = df["target"].notna() & df["persistence"].notna()
    df = df[valid].copy()
    ...
    y = df["target"]
    y_pers = df["persistence"]

    X_train, y_train = X[train_m], y[train_m]
    X_val, y_val = X[val_m], y[val_m]
    X_eval, y_eval = X[eval_m], y[eval_m]

    y_val_pers, y_eval_pers = y_pers[val_m], y_pers[eval_m]

    print(f"Train: {len(X_train):,}, Val: {len(X_val):,}, Eval: {len(X_eval):,}")

    # Persistence baseline
    pers_val_mae = mean_absolute_error(y_val, y_val_pers)
    pers_eval_mae = mean_absolute_error(y_eval, y_eval_pers)
...
def main():
    ...
    parser.add_argument("--run-id", default=f"q21_correct_{datetime.now().strftime('%Y%m%d_%H%M')}")
