import { ArrowRight, Check, CircleAlert, Database, Play, ShieldCheck, TrendingUp, AlertTriangle } from 'lucide-react'
...
function ChangeIndicator({ current, forecast }: { current: number; forecast: number }) {
  ...
  const percentChange = Math.abs(delta / current) * 100
  const isIncreasing = delta > 0
  const isSignificant = Math.abs(delta) > 0.5

  if (!isSignificant) return null

  return <span className={`change-indicator ${isIncreasing ? 'increasing' : 'decreasing'}`}>
    <TrendingUp size={14} style={{ transform: isIncreasing ? 'none' : 'scaleY(-1)' }} />
    {isIncreasing ? '+' : ''}{delta.toFixed(1)} ppm ({percentChange.toFixed(0)}%)
  </span>
}
...
export function ShiftView({ snapshot, q21Status, q21Runtime, onScenario }: { snapshot: Snapshot; q21Status: Q21Status | null; q21Runtime: Q21Runtime | null; onScenario: () => void }) {
  ...
  return <>
    <section className="shift-summary" aria-label="Сводка смены">
      <div className="summary-title"><span>ОПЕРАТИВНЫЙ СРЕЗ</span><strong>{snapshot.mode === 'live' ? 'Данные получены' : 'Нет технологических данных'}</strong></div>
      <div className="summary-item"><Database size={17} /><span>Буфер признаков<strong>{snapshot.buffer.points} точек · {Math.round(snapshot.buffer.duration_minutes)} мин</strong></span></div>
      <div className={`summary-item ${staleCount ? 'warning' : 'normal'}`}><CircleAlert size={17} /><span>Источники данных<strong>{staleCount ? `${staleCount} требуют внимания` : 'Все актуальны'}</strong></span></div>
      <div className={`summary-item ${snapshot.readiness.storage_ready ? 'normal' : 'warning'}`}><ShieldCheck size={17} /><span>Аудит решений<strong>{snapshot.readiness.storage_ready ? 'Запись активна' : 'Хранилище недоступно'}</strong></span></div>
    </section>
