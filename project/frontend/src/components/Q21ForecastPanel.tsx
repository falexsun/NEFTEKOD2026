...
export function Q21ForecastPanel({ runtime }: { runtime: Q21Runtime | null }) {
  ...
  return <section className="panel q21-forecast-panel">
    ...
    <div className="q21-shadow-summary">
      <span>Закрыто прогнозов: <b>{runtime.shadow_metrics.resolved}</b></span>
      {runtime.shadow_metrics.mae != null && <span>Общий MAE: <b>{formatNumber(runtime.shadow_metrics.mae, 2)} ppm</b></span>}
      {runtime.shadow_metrics.coverage != null && <span>Coverage: <b>{Math.round(runtime.shadow_metrics.coverage * 100)}%</b></span>}
      {runtime.shadow_metrics.by_horizon.map(metric => <span key={metric.horizon_hours}>MAE {metric.horizon_hours} ч: <b>{formatNumber(metric.mae, 2)} ppm</b>{metric.interval_coverage != null && <small> · покрытие {Math.round(metric.interval_coverage * 100)}%</small>}</span>)}
    </div>
