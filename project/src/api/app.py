...
# Q21 Shadow Pipeline metrics
Q21_SHADOW_READY = Gauge("neftekod_q21_shadow_ready", "1 when Q21 shadow pipeline is ready")
Q21_CURRENT_PPM = Gauge("neftekod_q21_current_ppm", "Current Q21 sulfur content in ppm")
Q21_FORECAST_1H_PPM = Gauge("neftekod_q21_forecast_1h_ppm", "Q21 forecast 1 hour ahead in ppm")
Q21_EXCEEDANCE_PROBABILITY = Gauge("neftekod_q21_exceedance_probability", "Probability of Q21 exceeding 10 ppm")
Q21_SHADOW_MAE = Gauge("neftekod_q21_shadow_mae_ppm", "Shadow pipeline MAE in ppm")
Q21_SHADOW_COVERAGE = Gauge("neftekod_q21_shadow_coverage", "Fraction of forecasts closed with actual Q21")
Q21_SHADOW_NO_ACTION = Gauge("neftekod_q21_shadow_no_action_count", "Count of NO_ACTION recommendations")
Q21_BUFFER_POINTS = Gauge("neftekod_q21_buffer_points", "Points in Q21 telemetry buffer")
Q21_DATA_FRESHNESS = Gauge("neftekod_q21_data_freshness_minutes", "Minutes since last Q21 point")
Q21_PREDICTIONS_COUNT = Counter("neftekod_q21_shadow_predictions_total", "Total Q21 shadow predictions made")
Q21_CLOSED_COUNT = Counter("neftekod_q21_shadow_closed_total", "Total Q21 forecasts closed with actual values")
...
def _init_runtime():
    ...
    global _anomaly_detector, _timeline, _telegram_notifier
    ...
    # Train anomaly detector on historical data if available
    if _runtime_store:
        try:
            ...
            if len(historical) > 100:
                ...
                logger.info(f"Anomaly detector trained on {len(historical)} historical points")
        except Exception as exc:
            logger.warning(f"Could not train anomaly detector: {exc}")
...
# Demo endpoints for hackathon finale
try:
    from src.api.demo_endpoints import router as demo_router
    app.include_router(demo_router)
    logger.info("Demo endpoints enabled for hackathon")
except ImportError:
    logger.info("Demo endpoints not available")
...
async def q21_telemetry(
    ...
    Q21_CURRENT_PPM.set(payload.q21)
    Q21_BUFFER_POINTS.set(len(history))
    if result.get("q21_forecast_1h") is not None:
        Q21_FORECAST_1H_PPM.set(result["q21_forecast_1h"])
    if result.get("exceedance_probability") is not None:
        Q21_EXCEEDANCE_PROBABILITY.set(result["exceedance_probability"])
    if result.get("action") == "NO_ACTION":
        Q21_SHADOW_NO_ACTION.inc()
    Q21_PREDICTIONS_COUNT.inc()
    if resolved > 0:
        Q21_CLOSED_COUNT.inc(resolved)

    # Add timeline event for forecast
    if result.get("q21_forecast_1h"):
        q21_change = result["q21_forecast_1h"] - payload.q21
        severity = "critical" if payload.q21 >= 10 else "warning" if abs(q21_change) > 1.0 else "info"

        _timeline.add_event(
            event_type="forecast_generated",
            severity=severity,
            title=f"Q21 прогноз: {payload.q21:.2f} → {result['q21_forecast_1h']:.2f} ppm",
            description=f"Изменение {q21_change:+.2f} ppm, вероятность превышения {result.get('exceedance_probability', 0)*100:.0f}%",
            data={"q21": payload.q21, "forecast": result["q21_forecast_1h"], "change": q21_change},
        )
    ...
    if payload.q21 >= 10.0:
        _timeline.add_event(
            ...
            severity="critical",
            title="Q21 превысил предел 10 ppm",
            description=f"Текущее значение: {payload.q21:.2f} ppm",
            data={"q21": payload.q21, "threshold": 10.0},
        )

        # Send Telegram alert asynchronously
        import asyncio
        asyncio.create_task(_telegram_notifier.send_q21_alert(
            q21_current=payload.q21,
            q21_forecast=result.get("q21_forecast_1h"),
...
async def q21_runtime(_: Identity = Depends(operator_access)):
    ...
    # Update Prometheus metrics from shadow pipeline state
    if points:
        Q21_BUFFER_POINTS.set(len(points))
        freshness_minutes = (utc_now() - latest_timestamp).total_seconds() / 60
        Q21_DATA_FRESHNESS.set(freshness_minutes)

    if shadow_metrics:
        if shadow_metrics.get("mae") is not None:
            Q21_SHADOW_MAE.set(shadow_metrics["mae"])
        if shadow_metrics.get("coverage") is not None:
            Q21_SHADOW_COVERAGE.set(shadow_metrics["coverage"])
...
async def q21_live_feed(websocket: WebSocket):
    """Real-time Q21 updates via WebSocket."""
        ...
        if _runtime_store:
            ...
            if latest:
                await websocket.send_json({
                    ...
                    "data": {
                        "q21_current": latest.get("Q21"),
                        "operating_mode": latest.get("operating_mode"),
                    }
                })
...
async def get_timeline_events(
    ...
    since = utc_now() - timedelta(minutes=since_minutes) if since_minutes else None
    event_types = [event_type] if event_type else None

    events = _timeline.get_recent_events(
        limit=limit,
        since=since,
...
@app.get("/timeline/root-cause")
async def get_root_cause_chain(
    timestamp: datetime,
    window_minutes: int = Query(default=60, le=180),
    _: Identity = Depends(operator_access),
):
    """Get chain of events leading to a specific moment."""
    chain = _timeline.get_root_cause_chain(
        _as_utc(timestamp),
    ...
    return {
        ...
        "window_minutes": window_minutes,
        "event_chain": [e.model_dump() for e in chain],
        "count": len(chain),
    }
...
async def detect_anomaly(
    ...
    if not _anomaly_detector or not _anomaly_detector.is_fitted:
        return {
            "is_anomaly": False,
            "reason": "Anomaly detector not trained",
            "anomaly_score": 0.0,
        }

    result = _anomaly_detector.detect(telemetry)

    # Add to timeline if anomaly
    if result["is_anomaly"]:
        _timeline.add_event(
            event_type="anomaly_detected",
            severity="warning",
            title="Аномалия в телеметрии",
            description=f"Обнаружена аномалия (score: {result['anomaly_score']:.2f})",
            data=result,
        )

        # Send Telegram alert if critical
        if abs(result["anomaly_score"]) > 1.0:
            asyncio.create_task(_telegram_notifier.send_anomaly_alert(
                description="Обнаружена критическая аномалия в телеметрии",
...
async def optimize_scenarios(
    baseline_timestamp: datetime,
    http_request: Request,
    max_iterations: int = Query(default=50, le=100),
    identity: Identity = Depends(operator_access),
):
    """Find optimal control scenarios using evolutionary algorithm."""
    ...
    readiness = _readiness()
    if not readiness["optimization_ready"]:
        raise HTTPException(status_code=503, detail="Модель сценариев не готова")

    controls = _control_registry.get_optimization_bounds()
    optimizer = get_scenario_optimizer()

    # Run optimization
    best_scenarios = optimizer.optimize_scenarios(
        current_state=state.model_dump(),
        controls=controls,
        surrogate_model=_agents["surrogate"],
        safety_agent=_agents["safety"],
    ...
    _timeline.add_event(
        ...
        severity="info",
        title="Оптимизация сценариев",
        description=f"Найдено {len(best_scenarios)} оптимальных сценариев",
        data={"scenarios_count": len(best_scenarios)},
    )
    ...
    result = {
        ...
        "best_q21": min(s["predicted_q21"] for s in best_scenarios) if best_scenarios else None,
    }

    if _runtime_store:
        decision_id = str(uuid.uuid4())
        _runtime_store.save_decision(
            decision_id, utc_now(), "scenario_optimization", result,
            http_request.state.request_id, identity.name
        )
        result["decision_id"] = decision_id
